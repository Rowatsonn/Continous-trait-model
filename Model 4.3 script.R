...
test <- Evolving_traits(generations = 2)

## Run 6 runs across 6 cores ## 
six_runs <- function(min_lifespan = 20, max_lifespan = 20, P = 0.5, min_verticaltransmission = 0, max_verticaltransmission = 0){
  # create a cluster with 6 workers
  cl <- makeCluster(6)
  
  # Create a snapshot of the R environment to pass to the nodes
  snapshot <- list(
    Evolving_traits = Evolving_traits,
    log_to_prob = log_to_prob,
    prob_to_log = prob_to_log
  )
  
  # Export the snapshot to each worker node in the cluster
  clusterExport(cl, varlist = c("snapshot"))
  ...
  for (i in 1:6) {
    params_list[[i]] <- list(
      ...
      P = P,
      min_verticaltransmission = min_verticaltransmission,
      max_verticaltransmission = max_verticaltransmission,
      run = i
    )
  }
  
  # run the model in parallel using clusterApply
  result_list <- clusterApply(cl, params_list, function(params){
    # Restore the snapshot on each worker node
    list2env(snapshot, .GlobalEnv)
    # Load tidyverse
    library(tidyverse)
    # Run the Evolving_traits function with the specified parameters
    Evolving_traits(generations = 100, min_lifespan = params$min_lifespan, max_lifespan = params$max_lifespan, P = params$P, run = params$run)
  })
