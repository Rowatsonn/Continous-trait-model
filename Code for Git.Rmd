...
1) Learning speed / brain power - This is a value from 0 - 1 that represents learning speed. 1 indicates that they learn as fast as they possibly could, 0 indicates there is no learning. 

2) Allocation strategy - Value from 0 - 1 that represents how much of their brain (see below) the individual dedicates to task A. 1 indicates a full A specialist, 0 indicates a full B specialist and 0.5 a full generalist. 

3) Learning generalisation - Value from 0 - 1 that represents how much individuals generalise their learning. 0 indicates they only learn about the task they are attempting and 1 indicates they fully generalise their learning between the tasks.

4) Brain size - Value from 0 - 1 that represents how good at the tasks the individuals can be. 1 indicates the maximum possible skill allowed by the model (log odds of 10) and 0 represents the minimum. 

5) Vertical transmission - Value from 0 - 1 that represents how efficient individuals are at vertical transmission. 1 represents maximally efficient (50%) and 0 indicates no vertical transmission. 

Baseline ability depends on brain size. If an individual is better at learning (higher brain power), then this comes at a trade off of a lower baseline. Allocation strategy then determines how much of their learning goes to A AND how much of their baseline goes to A. 

With maximum learning ability the baseline ability is -3 (4.74%), with minimum learning ability the baseline is 1 (73.11%). The maximum baseline ability with NO learning is. Therefore, baseline ability = 1 - (4 * learning speed).

In their lifespan, agents encounter L tasks. Each have the same payoff. We vary P (probability of A).

For simplicity, the cost parameter is identical for all the strategies (0.1). To figure out the cost of learning speed, brain size, learning generalisation, and vertical transmission you just do: Trait * 0.2 * Lifespan.

NOTE - The code currently does not properly implement costs for brain size. If we choose to let this evolve, we need to add costs. I have commented where functionality is missing. 

Also, vertical transmission mutation and lifespan mutation default to 0. 
...
Evolving_traits <- function(
  ...
  Difference = 1 - -3 # What is the difference between the minimum and maximum baselines?
  ...
  for(r in 1:runs){
    ...
    current_population <- tibble(total_payoff = 0,
      ...
      mutate(Lifespan = sample(lifespans, size = N, replace = T),
             Lifespan = ifelse(Lifespan > max_lifespan, Lifespan - 1, Lifespan), # This is to also solve the bug
    ...
    for(gen in 1:generations){
      ...
      end_population <- current_population %>%
        mutate(run = r, generation = gen, P = P)
      ...
      ### Vertical transmission ### Figure out the bonus inheritence to add later.
...
```
      ...
      # You want to try to do everything as vectors, without a for loop for agents.
      # it can get tricky, but it will let the code run much faster
      # Here's a few lines as examples, I'll put your lines in comments
      
      # total_payoff <- 0
      total_payoffs <- rep(0, N)
      
      # Tasks <- sample(c("A", "B"), size = L, replace = T, prob = c(P, 1-P))
      n_tasks_a <- rbinom(N, L, P)
      
      # Success <- ifelse(task == "A", rbinom(1, size = 1, prob = log_to_prob(A_prob)), rbinom(1, size = 1, prob = log_to_prob(B_prob)))
      success <- rbinom(N, n_tasks_a, log_to_prob(current_population$prob_A)) + rbinom(N, L-n_tasks_a, log_to_prob(current_population$prob_B))
